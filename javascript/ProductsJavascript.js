...
var counter = 0; /*for checking the position of the elements inside the for loop*/

var hoodiesLink = document.getElementById("hoodies-link");
var jumpersLink = document.getElementById("jumpers-link");
var tShirtsLink = document.getElementById("t-shirt-ink");

var itemsBlock = document.getElementById("products-menu");
...
var strTile = "this is the title of the item!!";
...
// that is clicked.
    ...
    newdiv.innerHTML = `
       ...
       <div class="Imgdiv">
            <img src= ${i[5]} alt="The image cannot be fount!!">
       </div>
        <div class="productsdiv">
               <h2 class="title">${strTile + " - " + i[2]}</h2>
               <a class="description" onclick="UpdateSession(${counter})" href="item.html"> ${i[3]}. Read more...</a>
               <p class="price">${i[4]}</p>
               <button onclick="UpdateLocal(${counter})" id="buyButton">Buy</button>
        </div>
       
       `;


    /*this div is for place id at the first item in each category to jump to that each category in the page */

    if (counter === 0){
        newdiv.setAttribute("id","first-hoodie");
    }

    else if (counter === 35){
        newdiv.setAttribute("id","first-jumper");
    }
    else if (counter === 74){
        newdiv.setAttribute("id","first-tshirt");
    }
...
/*this is for link to each id so the page can jump to each category*/

hoodiesLink.setAttribute("href" , "#first-hoodie");
jumpersLink.setAttribute("href" , "#first-jumper");
tShirtsLink.setAttribute("href" , "#first-tshirt");



// this function is for overwrite inside the session storage the item when read more is clicked. the stringify
// command converts the hole  [] array of the item into string, the only type thet session storage accepts
function UpdateSession (item){

    sessionStorage.setItem('SessionItem', JSON.stringify(items[item]));

}



// this function is for add the element that user clicked to buy
// to the local storage and every time check for duplicates
function UpdateLocal (counter){

    var key = items[counter][1] + "(" + items[counter][0] + ")";

    checkForDuplicates(key, counter);
...
function checkForDuplicates(key, counter) {

    if (localStorage.getItem(key) !== null) {
        checkForDuplicates(key+="(1)",counter);
    }
    localStorage.setItem(key, JSON.stringify(items[counter]));
}
